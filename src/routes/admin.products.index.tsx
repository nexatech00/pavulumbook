    ...
    <div>
      ...
      <div className="mt-8 overflow-hidden rounded-2xl border border-border bg-card">
        <table className="w-full text-left">
          ...
          <tbody>
            {isLoading && (
              <tr><td colSpan={5} className="px-5 py-8 text-center text-charcoal/60">Loading…</td></tr>
            )}
            {!isLoading && products.length === 0 && (
              <tr><td colSpan={5} className="px-5 py-8 text-center text-charcoal/60">No products yet.</td></tr>
            )}
            {products.map((p) => (
