...
export const Route = createFileRoute("/checkout")({
  head: () => ({
    meta: [
      { title: "Checkout — Pavulum" },
      { name: "description", content: "Complete your Pavulum order." },
    ],
    ...
    return (
      <SiteLayout>
        <div className="mx-auto max-w-xl px-6 py-32 text-center">
          ...
          <Link to="/shop" className="mt-6 inline-block text-terracotta hover:underline">
            Visit the shop →
          </Link>
        </div>
      </SiteLayout>
    ...
    return (
      <SiteLayout>
        <div className="mx-auto max-w-xl px-6 py-24 text-center">
          <p className="text-xs uppercase tracking-[0.2em] text-soft-gold">Order confirmed</p>
          ...
          {hasDigital && (
            <div className="mt-8 rounded-2xl bg-secondary/70 p-6 text-left">
              <h2 className="font-serif text-xl text-deep-brown">Your digital downloads</h2>
                ...
                {detailed
                  ...
                  .map((l) => (
                    <li key={l.product.id} className="flex items-center justify-between">
                      ...
                      <a href="#" className="text-terracotta hover:underline">Download</a>
                    </li>
                  ))}
              </ul>
            </div>
          )}
          <Link to="/" className="mt-10 inline-block text-terracotta hover:underline">
            Back home →
          </Link>
        </div>
      </SiteLayout>
  ...
  return (
    <SiteLayout>
      <div className="mx-auto max-w-5xl px-6 py-16">
        ...
        <p className="mt-2 italic text-charcoal/60">Test mode · no card will be charged.</p>
            ...
            <ul className="mt-4 space-y-3">
              {detailed.map((l) => (
                <li key={l.product.id} className="flex items-center gap-3">
                  ...
                  <p className="text-sm text-charcoal/85">${l.lineTotal.toFixed(2)}</p>
                </li>
              ))}
            </ul>
            <div className="mt-5 space-y-2 border-t border-border pt-4 text-sm">
              <div className="flex justify-between text-charcoal/80"><span>Subtotal</span><span>${subtotal.toFixed(2)}</span></div>
              <div className="flex justify-between text-charcoal/80"><span>Shipping</span><span>{hasPhysical ? `$${shipping.toFixed(2)}` : "Free"}</span></div>
              <div className="flex justify-between pt-2 text-base text-deep-brown"><span>Total</span><span>${total.toFixed(2)}</span></div>
            </div>
          </aside>
        </div>
      </div>
