...
export const Route = createFileRoute("/cart")({
  head: () => ({
    meta: [
      { title: "Cart — Pavulum" },
      { name: "description", content: "Your Pavulum cart." },
    ],
          ...
          <div className="mt-10 grid gap-12 md:grid-cols-[1fr_320px]">
                  ...
                  <div className="flex flex-1 flex-col">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-charcoal/60 capitalize">{product.category}</p>
                      </div>
            ...
            <aside className="h-fit rounded-2xl bg-card p-6 shadow-sm shadow-deep-brown/5">
              ...
              <dl className="mt-5 space-y-2 text-sm">
                ...
                <Row label={hasPhysical ? "Shipping (est.)" : "Shipping"} value={hasPhysical ? `$${shipping.toFixed(2)}` : "Free"} />
...
function Row({ label, value, bold }: { label: string; value: string; bold?: boolean }) {
  return (
    <div className={`flex items-center justify-between ${bold ? "text-base text-deep-brown" : "text-charcoal/80"}`}>
      <dt>{label}</dt>
      <dd>{value}</dd>
    </div>
  );
