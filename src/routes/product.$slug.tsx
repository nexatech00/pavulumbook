...
export const Route = createFileRoute("/product/$slug")({
  head: ({ params }) => ({
    meta: [
      { title: `${params.slug} — Pavulum` },
      { property: "og:title", content: `${params.slug} — Pavulum` },
    ],
...
function ProductPage() {
  ...
  if (isLoading) {
    return (
      <SiteLayout>
        <div className="mx-auto max-w-2xl px-6 py-32 text-center text-charcoal/60">Loading…</div>
      </SiteLayout>
    );
  }
  ...
  return (
    <SiteLayout>
      <div className="mx-auto max-w-6xl px-6 py-16">
        <Link to="/shop" className="mb-10 inline-block text-sm text-charcoal/60 hover:text-terracotta">
          ← Back to shop
        </Link>
        <div className="grid gap-12 md:grid-cols-2">
          ...
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-soft-gold">{product.category}</p>
            ...
            <p className="mt-6 leading-relaxed text-charcoal/85">{product.longDescription}</p>
